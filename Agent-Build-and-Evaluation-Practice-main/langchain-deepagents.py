...
# ---------------------------------------------------------------------------
# Observation(트레이싱) — LangSmith
# ---------------------------------------------------------------------------
# langchain/langgraph 네이티브라 코드 변경 없이 환경변수만으로 켜진다. 켜지면 이
# 에이전트의 모든 LLM 호출·tool 호출·subagent 위임이 각각 observation(개별 실행
# 기록)으로 LangSmith 대시보드에 남는다: input/output, latency, token/cost, tool
# call, error. meta-harness 의 격리 실행(headless run)도 같은 프로세스 안에서
# 이 모듈을 import 하므로 자동으로 트레이싱 대상에 포함된다.
# smith.langchain.com 에서 프로젝트를 만들고 API 키를 .env 의 LANGCHAIN_API_KEY 에
# 넣으면 활성화된다. 안 넣으면 트레이싱 없이 평소대로 동작한다(옵트인).
...
if _langsmith_key:
    ...
    os.environ.setdefault("LANGCHAIN_PROJECT", os.getenv("LANGCHAIN_PROJECT", "plc-vibe-agent"))
    print(f"[Observation] LangSmith 트레이싱 활성화 — project={os.environ['LANGCHAIN_PROJECT']}")
else:
    print("[Observation] LangSmith 비활성화 (.env 의 LANGCHAIN_API_KEY 를 채우면 트레이싱이 켜집니다)")
